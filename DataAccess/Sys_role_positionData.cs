...
{
    public class Sys_role_positionData : BaseData
    {
        /// <summary>
        /// 對應的Model型別
        /// </summary>
        private Type _modelType = typeof(Sys_role_positionInfo);
        ...
        /// <returns></returns>
        public List<Sys_role_positionInfo> GetListByRoleUnit(string sys_rid, string sys_uid)
        {
            string sql = @"
                select *
                from sys_role_position rp
                where rp.sys_rid = @sys_rid
	                and (rp.sys_uid = '*' or rp.sys_uid = @sys_uid)
                order by rp.sys_uid, sys_seq";
