...
{
    public class Sys_loginldapData : BaseData
    {
        /// <summary>
        /// 對應的Model型別
        /// </summary>
        private Type _modelType = typeof(Sys_loginldapInfo);
        ...
        /// <returns></returns>
        public Sys_loginldapInfo GetInfo(string id)
        {
            Sys_loginldapInfo info = null;
            ...
            var info_lst = Db.GetEnumerable<Sys_loginldapInfo>(sql, Db.GetParam("@id", id)).ToList();
            if (info_lst.Count() > 0)
                info = info_lst[0];
