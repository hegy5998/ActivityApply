...
{
    public class Sys_announceData : BaseData
    {
        /// <summary>
        /// 對應的Model型別
        /// </summary>
        private Type _modelType = typeof(Sys_announceInfo);
        ...
        #region 取得可以顯示於系統選單公告的資料
        /// <summary>
        /// 取得可以顯示於系統選單公告的資料
        /// </summary>
        /// <returns>所有資料</returns>
        public IEnumerable<Sys_announceInfo> GetEnableList()
        {
            ...
            sql_sb.Append(@"
                ...
                from sys_announce
                where sys_enable = 'Y'
                    and sys_date <= (" + Db.DbNowTimeSQL +  @")
                order by sys_date desc, updtime desc");

            foreach (var item in Db.GetEnumerable<Sys_announceInfo>(sql_sb.ToString()))
            {
                yield return item;
            }
        }
