...
{
    public class Sys_moduleData : BaseData
    {
        /// <summary>
        /// 對應的Model型別
        /// </summary>
        private Type _modelType = typeof(Sys_moduleInfo);
        ...
        /// <returns></returns>
        public List<Sys_moduleInfo> GetListBySystem(string sys_id)
        {
            string sql = @"                
                select sys_module.*,
					sys_system.sys_name
                from sys_module, sys_system
				where sys_module.sys_id = sys_system.sys_id
                    and sys_module.sys_id = @sys_id
                order by sys_system.sys_id, sys_module.sys_mid";

            var data_lst = Db.GetEnumerable<Sys_moduleInfo>(sql, Db.GetParam("@sys_id", sys_id)).ToList();
        ...
        /// <returns></returns>
        public int UpdateSysId(IDbTransaction trans, string oldSysId, string newSysId)
        {
            string sql = @"
                update sys_module
                set sys_id = @newSysId
                where sys_id = @oldSysId";

            var param_lst = new List<IDataParameter>() { 
                Db.GetParam("@oldSysId", oldSysId),
                Db.GetParam("@newSysId", newSysId)
            };
