...
{
    public class Sys_processcontrolData : BaseData
    {
        /// <summary>
        /// 對應的Model型別
        /// </summary>
        private Type _modelType = typeof(Sys_processcontrolInfo);
        ...
        #region 取得某作業代碼的子功能資料
        /// <summary>
        /// 取得某作業代碼的子功能資料
        /// </summary>
        /// <param name="sys_pid">作業代碼</param>
        /// <returns></returns>
        public List<Sys_processcontrolInfo> GetList(string sys_pid)
        {
            StringBuilder sql_sb = new StringBuilder();
            sql_sb.Append(@"
                select *
                from sys_processcontrol 
                where sys_pid = @sys_pid
                order by sys_cid");
            ...
            var data_lst = Db.GetEnumerable<Sys_processcontrolInfo>(sql_sb.ToString(), param_lst.ToArray()).ToList();
