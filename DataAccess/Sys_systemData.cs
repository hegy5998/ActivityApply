...
{
    public class Sys_systemData : BaseData
    {
        /// <summary>
        /// 對應的Model型別
        /// </summary>
        private Type _modelType = typeof(Sys_systemInfo);
        ...
        /// 取得所有資料
        /// </summary>
        /// <param name="isOnlyShowHasModule">是否只取得有模組的系統資料</param>
        /// <returns>系統資料</returns>
        public List<Sys_systemInfo> GetList(bool isOnlyShowHasModule = false)
        {
            StringBuilder sql_sb = new StringBuilder();
            sql_sb.Append(@"select * from sys_system s");

            if (isOnlyShowHasModule)
                sql_sb.Append(" where s.sys_id in (select distinct sys_id from sys_module m where m.sys_id = s.sys_id)");

            sql_sb.Append(" order by s.sys_id");

            var data_lst = Db.GetEnumerable<Sys_systemInfo>(sql_sb.ToString()).ToList();
