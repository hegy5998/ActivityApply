...
{
    public class Sys_configData : BaseData
    {
        /// <summary>
        /// 對應的Model型別
        /// </summary>
        private Type _modelType = typeof(Sys_configInfo);
            ...
            if (res.IsSuccess)
            {
                string sql = @"
                    insert into [" + _modelType.GetTableName() + @"] 
                        (" + Db.GetSqlInsertField(_modelType, data_dict) + @", [updid], [updtime]) 
                    values (" + Db.GetSqlInsertValue(data_dict) + ", '" + loginUser.Act_id + "'"+ ", (" + Db.DbNowTimeSQL + ")" + ")";
        ...
        /// </summary>
        /// <returns>資料</returns>
        public List<Sys_configInfo> GetList()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("select * from sys_config order by sys_name");

            var data_lst = Db.GetEnumerable<Sys_configInfo>(sb.ToString()).ToList();
